...
<div class="w-100 d-flex pl-0">
                                <img id="roomImageChatBar" class="rounded-circle shadow avatar-sm mr-3 chat-profile-picture" src="https://user-images.githubusercontent.com/35243461/168796877-f6c8819a-5d6e-4b2a-bd56-04963639239b.jpg" />
                                <div class="mr-3">
                                    <a href="!#">
                                        <p id="roomNameChatBar" class="fw-400 mb-0 text-dark-75">Beate Lemoine</p>
                                    </a>
                                    <p id="roomLastestUpdateChatBar" class="sub-caption text-muted text-small mb-0"><i class="la la-clock mr-1"></i>last seen today at 09:15 PM</p>
                                </div>
                            </div>
*/
function getRoomInformation(roomId) {
    $.get("http://localhost:8888/getRoomInfo?token=" + token + "&roomId=" + roomId, function (data) {
        ...
        if (!data.roomType) {
            if (data.user1 == userId) {
                chatRoomImage = RealetedUserMap.get(data.user2).username;
            } else {
                chatRoomImage = RealetedUserMap.get(data.user1).username;
            }
        }
...
function getUserRoomInfomation(roomId) {
    $.get("http://localhost:8888/getUserRoomInfo?token=" + token + "&roomId=" + roomId, function (data) {
        ...
        for (let i = 0; i < data.length; i++) {
            userRoom.set(userRooms[i].id, userRooms[i]);
        }
    }).fail(function (error) {
        console.log("Error form getRoomInformation() with roomId " + roomId + " " + error);
    })
...
function changeSelectedRoom(roomId) {
    if (selectedRoom != roomId) {
        ...
        // getUserRoomInfomation(roomId).then(function(){
        //     getChatMessageByPage(roomId, 1, dateToGenerateChat).then(function(){
        //         getRoomInformation(roomId);
        //     })
        // });
        //Change the global selectedRoomId
        selectedRoom = roomId;
    }
}

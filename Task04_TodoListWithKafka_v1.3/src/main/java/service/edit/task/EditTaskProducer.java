...
public class EditTaskProducer extends Thread{
    ...
    public void run() {
        ...
        Gson gson = new Gson();
        producerProperties.setProperty(ProducerConfig.CLIENT_ID_CONFIG, "edit-task-producer");
        ...
        while (true) {
            if (EditTaskData.data.size() > 0) {
                for (Map.Entry<Integer, Request> data : EditTaskData.data.entrySet()) {
                    ...
                    if (val.getMethod().equalsIgnoreCase("post")) {
                        ...
                        if (isAuthenticated) {
                            ...
                            Todo todo = TodoDAO.updateTitleAndReturn(u1.getId(), taskId, title);
                            if (todo != null) {
                                try {
                            ...
                            response.setTextData("Edit Success");
